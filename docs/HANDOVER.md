...
- **2026-08-14 動態戰力偵測與三階梯據點難度生成 (New!)**：
  - **戰力修復**：修正 `Adventurer.power` 屬性計算，整合八維屬性與裝備攻防加成。
  - **人數與戰力雙重適應**：`MapEventSystem.ts` 探索生成據點時，動態偵測玩家最強 $N$ 人戰力（$N = \min(5, \text{現有傭兵數})$）。剛開局 1 位傭兵時只採計 1 人戰力，並限制怪物數量為 1~2 隻（以 1 隻為主），杜絕開局 1v5 群毆滅團。
  - **三階梯機率對標**：50% 勢均力敵 (0.85~1.05x)、35% 越級挑戰 (1.15~1.35x)、15% 凶險精英 (1.60~2.00x)。
