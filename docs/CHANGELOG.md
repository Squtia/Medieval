...
## [2026-07-22] 冒險者卡片 Tooltip 資訊精簡與換行置中排版
...
## [2026-07-22] 冒險者小隊 ICON 高光調亮與大陸地圖視圖隔離

- **冒險者小隊 ICON 與視覺增強**：
  - **圖示色彩與高光調亮**：為右下角快捷列的冒險者小隊按鈕 (`.dock-btn`) 加上閃耀金色金屬邊框 (`#eab308`)、高亮漸層背景與 `👥` 圖示金色發光陰影特效 (`text-shadow: 0 0 8px #fef08a`)，徹底解決原本顏色太暗沉的問題。
- **大陸地圖視圖隔離邏輯**：
  - **大陸地圖下隱藏與自動關閉**：玩家位於「大陸地圖 (`map-view`)」時，自動隱藏右下角冒險者隊伍按鈕 (`#btn-dock-party`) 並關閉冒險者小隊面板 (`#modal-party-list`)。
  - **據點街道專屬**：冒險者小隊面板與快捷按鈕僅在玩家進入「據點街道 / 國家視圖 (`scene-view`)」或據點設施時呈現，符合地圖視圖隔離設計。

- **冒險者小隊面板 UI 史詩升級**：
  - **HP / MP 能量條位置調整**：將原本位於右側屬點分頁 (`party-tab-viewport`) 內的 HP (深紅) 與 MP (寶藍) 能量條移出，整合至左側 **半身像立繪卡 (`party-portrait-card`) 的正下方** 顯示，包含居中動態陰影數字與平滑條狀比例，使半身像與冒險者當前狀態緊密連結，右側屬性/配點介面更為乾淨清晰。
  - **方案 A Full-Art 滿版卡牌質感重構**：
    ...
    - 冒險者姓名與等級改為半透明沉底印記，兼具 100% 畫面可讀性與現代手遊/RPG 的精緻實體卡牌質感，預先為未來高精細 2D/3D 立繪資產預留最大的展現空間。
  - **面板高度與防遮擋優化**：
...
- **冒險者/傭兵小隊面板精簡與 UI 重構**：
  ...
  - **動態冒險者姓名標題**：移除原本寫死的「🛡️ 傭兵小隊」靜態文字，頂部標頭改為即時顯示當前選取冒險者的姓名與等級職業（例如：`🛡️ 里奧弗里德 · 萊森 (Lv.1 見習騎士)`）。
  ...
  - **2/3 + 1/3 二分法排版**：
    - 上半部 (2/3 空間, ~285px)：左側常駐顯示冒險者肖像/半身像與狀態；中間動態切換詳情 Viewport（能量條、六維與裝備）；右側配置 `📊 屬點` 與 `⚔️ 武具` 垂直頁籤。
...
- **美術資產與 UI/UX 史詩重構**：
  ...
  - **街道右側欄目整頓**：自街道右側 Imperial Dashboard 徹底移除占用空間的「冒險者陣列」，收納至獨立的 `🛡️ 傭兵小隊 Modal` 中；讓右側欄目專注於爵位施政進度、每日收支與廣角歷史日誌，顯著提升整體版面質感與可讀性。
