...
export class NodeDetailModalController {
  ...
  public openRadialMenu(node: MapNode, clientX: number, clientY: number) {
    const radialMenu = document.getElementById('radial-menu');
    ...
    buttons.push({ icon: '🔍', text: '檢視據點詳情', action: () => this.openNodeDetailPanel(node) });

    if (node.ownerFactionId !== null && !node.isPlayerBase) {
      buttons.push({ icon: '🕵️', text: '派遣斥候 (開發中)', action: () => {}, disabled: true });
    }
    if (node.ownerFactionId === null && !node.isPlayerBase) {
      if (node.nodeLevel === NodeLevel.WILDERNESS) {
        buttons.push({ icon: '⛺', text: '在此建立營地', action: () => {
    ...
    if (!node.isPlayerBase) {
      if (node.ownerFactionId !== null) {
        buttons.push({ icon: '⚔️', text: '派遣軍隊攻城', action: () => { /* will hook up DispatchModal */ } });
      } else {
        buttons.push({ icon: '🛡️', text: '派遣傭兵討伐', action: () => { /* will hook up DispatchModal */ } });
      }
    }
